...
This application uses a MySQL database hosted on Clarkson University's server. Credentials and connection parameters are managed via the config.yml file (excluded from GitHub for security). Below is a sample of the configuration used during development:
...
⚠️ Note: This configuration is not included in the GitHub repository. You must create your own config.yml with your specific credentials to run the app locally.
